  ...
  const { form, message, errors, allErrors, enhance } = superForm(sourceForm, {
    ...
    onSubmit: ({ cancel }) => {
      if ($form.subdomain !== user?.subdomain) {
        toast.error('Subdomain does not match. Please enter your current subdomain to confirm.');
        cancel();
      }
    },
    ...
    onError: (error) => {
      ...
      toast.error('Failed to delete subdomain. Try again later.');
    }
  });
  ...
  >
    ...
    >
      <div class="flex items-center justify-between border-b p-4">
        <h2 class="text-xl font-bold">Delete Subdomain</h2>
        <button
          onclick={() => (subdomainPopupOpen = false)}
      ...
      <form action="?/deleteSubdomain" method="POST" use:enhance>
        <div class="flex flex-col gap-4 p-6">
          <p>
            When you delete your subdomain, it will be released and made available for others to
            claim. If someone else claims it, you won't be able to get it back. You can create a new
            subdomain at any time.
          </p>

          <p class="text-sm text-zinc-500 dark:text-zinc-400">
            Current Subdomain: <strong>{user?.subdomain || 'None'}</strong>
          </p>

          <label for="subdomain" class="text-sm font-medium">Enter subdomain to confirm</label>
          <input
            id="subdomain"
            name="subdomain"
            type="text"
