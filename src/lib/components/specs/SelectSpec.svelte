<script lang="ts">
  ...
  let {
    ...
    fields: InferSelectModel<typeof specificationField>[];
    onSelect?: (field: string) => void;
    overlayOpen: boolean;
  } = $props();

  $effect(() => {
    if (createSpecField.result) {
...
<FullscreenOverlay title="Select Field" Icon={List} zIndex={60} gap={8} bind:overlayOpen>
  <div class="flex items-center gap-2">
    <span
      class="shrink-0 bg-zinc-100 pr-1 text-sm text-zinc-400 dark:bg-zinc-800 dark:text-zinc-500"
    >
      CREATE FIELD
    </span>
  ...
  <form {...createSpecField} class="mb-2 flex w-full items-center justify-center gap-2">
    <input
  ...
  {#if fields.length > 0}
    {#each fields as field (field.key)}
      <button
        class="w-full cursor-pointer rounded-lg p-2 text-start transition-colors hover:bg-zinc-200 dark:hover:bg-zinc-200"
        onclick={() => {
          onSelect(field.id);
          overlayOpen = false;
        }}
